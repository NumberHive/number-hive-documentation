...
| `play`'s API origin (for `game` to call cross-origin) | **`https://backend.numberhive.app`** in production — confirmed by direct check against the live Render deployment (2026-07-26), and corroborated by `number-hive-complete/backend/.env.prod`'s real live secrets (`server_base_url=https://backend.numberhive.app`) and today's `docs/adr/001-free-game-infrastructure.md`. **Two prior candidate values were wrong and should not be used**: `api.numberhive.org` (an early `number-hive-newvis` design-draft guess — doesn't appear in either repo) and `api.numberhive.app` (this doc's own first correction attempt, sourced from `frontend/env.ts`'s hardcoded *fallback* default, not a confirmed deployed value). `number-hive-complete`'s own repo has **three parallel, mutually-contradicting deployment configs** (`render.yaml` with custom domains commented out; an apparently-abandoned Kubernetes/Pulumi config claiming `api.numberhive.org`; and the live Docker+SSH path using `.env.prod`'s `backend.numberhive.app`), and `CLAUDE.md` itself still says the production URL is "TBD" — that inconsistency is a separate infra-hygiene issue worth raising with whoever owns `number-hive-complete`'s deploy config, out of scope for this doc beyond flagging it here. `game`'s new `playApiUrl.ts` must resolve to an absolute origin (unlike `game`'s own `apiUrl.ts`, which safely falls back to a relative path only because it's same-origin with its own API) — a relative-path fallback here would silently hit `game`'s own domain instead of `play`'s. | `game` |
...
- 2026-07-26 (eighth follow-up) — **Correction to the immediately preceding entry.** The user
  checked the live Render deployment directly and reported `play`'s backend is actually served from
  `https://backend.numberhive.app`, not `api.numberhive.app`. Investigated further: the
  `api.numberhive.app` value in the seventh-follow-up entry/§6 was itself wrong — sourced from
  `frontend/env.ts`'s hardcoded fallback default, not a confirmed deployed value. Direct evidence
  (`backend/.env.prod`'s live production secrets, corroborated by today's
  `docs/adr/001-free-game-infrastructure.md`) confirms `backend.numberhive.app` is correct.
  Discovered along the way that `number-hive-complete` has three parallel, disagreeing deployment
  configs (Render, an apparently-abandoned Kubernetes/Pulumi setup, and the live Docker+SSH path),
  and its own `CLAUDE.md` still lists the production URL as "TBD" — flagged in §6 as a separate
  infra-hygiene issue for whoever owns that repo's deploy config, not resolved here. §6's origin row
