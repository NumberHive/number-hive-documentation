...
| `staging-local.numberhive.org` | Self-hosted staging environment for the education app (nginx → Tailscale → Docker Compose, all on personally-hosted infrastructure: frontend, backend, MongoDB, Temporal, PostgreSQL) | `number-hive-complete` | **Live** — replaced the old Render.com staging deployment, which is now fully decommissioned (per `STAGING-LOCAL.md`); tracks the `staging` branch |
