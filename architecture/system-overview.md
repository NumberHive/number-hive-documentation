...
| **Company operations / admin** | `number-hive-admin` (proposed — not yet created) | Proposed, not implemented | Billing/subscriptions (Stripe), customer/organisation records, user administration, projects/tasks. Currently lives *inside* `number-hive-complete` (`backend/src/graphql/admin/`, `frontend/.../AdminPanel/`) — ADR-005 (in `number-hive-complete`) proposes extracting it into its own service. |
...
| Subscriptions, billing, customer/org records, projects/tasks | `number-hive-admin` (proposed; today: `number-hive-complete`) | `number-hive-complete` (entitlement projection only, via event push), `amber` (scoped read API) |
