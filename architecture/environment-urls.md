...
| Staging — **staging-local (personally-hosted, canonical)** | `https://staging-local.numberhive.org` | `https://staging-local.numberhive.org/graphql` (same origin, path-routed — `/graphql` and `/health` go to the backend container, everything else to frontend) | `STAGING-LOCAL.md` lines 39–43 |
...
decommissioned in favour of `staging-local.numberhive.org` (personally hosted). Given the same repo just
...
| Dev — personal preview box (developer-provided infrastructure) | `https://nhvis.puddicombe.com` → Tailscale IP `100.64.21.9:20056`, whatever dev-server preview is currently running; **502 when no preview is active** | same origin, `/api` and `/v1` proxied | `DEPLOY.md` line 14 |
| Staging — **Personally-hosted Docker Compose (long-standing canonical)** | *(personally-hosted staging; specific domain intentionally not documented here)* | bundled with frontend, same container/path | `DEPLOY.md` lines 15, 132 |
...
| Dev — personal preview box (developer-provided infrastructure) | `https://<personal-dev-box>:20167` (dynamic reserved port) | `/api/*` proxied through the same origin (internal port `20166`) | `dev_server_status`, 2026-08-01 |
...
still running on a personally-hosted dev box, just stale and unused. `number-hive-admin`'s Lead confirmed its most
